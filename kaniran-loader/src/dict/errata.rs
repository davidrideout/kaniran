//! Load-time errata for `ichiran/dict` (`dict-errata.lisp`).
//!
//! The runtime errata constants, hooks, and predicates stay in
//! `kaniran_core::dict::errata`; this module carries the async,
//! Postgres-writing fixups applied after the JMdict load completes
//! (the `add-errata-*`, `add-conj*`, reading/sense/gloss editors).

use crate::custom::load::{load_custom_data, CustomDataKey, LoadCustomDataError};
use crate::dict::load::conjugate::conjugate_entry_outer;
use crate::dict::load::jmdict::{add_new_sense, next_seq};
use crate::dict::load::readings::{set_reading, SetReadingObj};
use kaniran_core::characters::char_class::{test_word, CharClass};
use kaniran_core::conn::kani_context::KaniranContext;
use kaniran_core::dict::dao::{
    ConjSourceReading, Conjugation, Entry, Gloss, KanaText, KanjiText, SenseProp,
};
use kaniran_core::dict::errata::root_diff_fn;
use kaniran_core::dict::grammar::suffix::rules::apply_patch;
use kaniran_core::dict::kani_reading_table::KaniReadingTable;
use sqlx::Row;
...
pub async fn add_errata(ctx: &KaniranContext) -> Result<(), LoadCustomDataError> {
    ...
    let mut entry: kaniran_core::dict::dao::Entry = sqlx::query_as("SELECT * FROM entry WHERE seq = $1")
        .bind(2611370)
        .fetch_one(ctx.pool.as_ref().expect("postgres pool"))
        .await?;
    entry.root_p = false;
