//! Load-time conjugation writers for `ichiran/dict` (`dict-load.lisp`).
//!
//! The sync conjugation helpers (`construct_conjugation`,
//! `conjugate_word`, `lex_compare`) stay in
//! `kaniran_core::dict::load::conjugate`; this module carries the
//! async, Postgres-writing builders that materialize the conjugation
//! tables (`conjugate_entry_*`, `insert_conjugation`,
//! `load_conjugations`, `load_secondary_conjugations`).

use crate::dict::errata::get_all_readings;
use crate::dict::load::jmdict::next_seq;
use kaniran_core::conn::kani_context::KaniranContext;
use kaniran_core::dict::load::conj_rules::{
    get_conj_rules, DO_NOT_CONJUGATE, DO_NOT_CONJUGATE_SEQ, POS_WITH_CONJ_RULES,
    SECONDARY_CONJUGATION_TYPES, SECONDARY_CONJUGATION_TYPES_FROM,
};
use kaniran_core::dict::load::conjugate::{construct_conjugation, lex_compare};
use kaniran_core::dict::load::pos::{get_pos, get_pos_index};
use sqlx::Row;
...
pub async fn load_secondary_conjugations(
    ...
    let to_conj: Vec<(i32, i32, i32)> = {
        ...
        q.fetch_all(ctx.pool.as_ref().expect("postgres pool")).await?
    };
...
/// Drop-in copy of `kaniran_core::dict::load::conjugate`'s private
/// `dedupe_keep_last`: keep only the last occurrence of each value,
/// preserving order. Lives here because the core copy is module-private
/// and the load-time builders that moved to this crate still need it.
