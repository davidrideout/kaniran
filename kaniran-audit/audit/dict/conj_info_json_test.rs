...
//!     --path corpus/<tag>/dict/conj_info_json.parquet

#[path = "../common/mod.rs"]
mod common;

use kaniran_core::conn::kani_context::KaniranContext;
use kaniran_core::dict::conj_data::conj_info_json;
...
/// Value following the keyword string `kw` in the captured flat arg plist
/// `[seq, ":CONJUGATIONS", v, ":TEXT", v, ":HAS-GLOSS", v]`.
...
/// Deep-sort arrays and object keys so comparison ignores SQL row order.
/// Lengths and contents are preserved, so a genuine count/content
/// divergence survives while pure reorderings collapse to equal.
...
/// First structural divergence as a JSON-path string (post-normalize).
