...
//! Source under test: `src/dict/suffix_sou.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix_rules::suffix_sou;
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_sou(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_sou: {}", e))?;
