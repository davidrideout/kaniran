//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-TEII`.
//! Source under test: `src/dict/suffix_teii.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_teii_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_teii.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-TEII` is a `def-simple-suffix`
//! (`dict-grammar.lisp:414`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_teii;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-TEII";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_teii(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_teii: {}", e))?;
