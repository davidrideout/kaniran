...
//!
//! Replays a captured `(root, sv, kf)` triple through the
//! `suffix-kudasai` `def-simple-suffix` (`dict-grammar.lisp:404`) and
...
//! Every captured call in the chunk-C corpus returns nil, so the
//! `Constant(360)` score-mod path is never actually compared.
...
use kaniran_core::dict::grammar::suffix::rules::suffix_kudasai;
