//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-KUDASAI`.
//! Source under test: `src/dict/suffix_kudasai.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_kudasai_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_kudasai.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-KUDASAI` is a `def-simple-suffix`
//! (`dict-grammar.lisp:404`). The macro signature is `(root sv kf)`,
...
//!
//! This suffix's `:score` is a `(constantly 360)` closure, carried in
...
const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-KUDASAI";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_kudasai(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_kudasai: {}", e))?;
