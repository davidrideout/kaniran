//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-NEBA`.
//! Source under test: `src/dict/abbr_neba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_neba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_neba.parquet
//!
//! `ICHIRAN/DICT:ABBR-NEBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:638`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_neba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_neba: {}", e))?;
