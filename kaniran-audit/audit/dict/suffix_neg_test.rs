//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-NEG`.
//! Source under test: `src/dict/suffix_neg.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix_rules::suffix_neg;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-NEG";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_neg(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_neg: {}", e))?;
