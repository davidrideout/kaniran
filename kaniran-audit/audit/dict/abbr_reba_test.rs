//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-REBA`.
//! Source under test: `src/dict/abbr_reba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_reba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_reba.parquet
//!
//! `ICHIRAN/DICT:ABBR-REBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:629`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_reba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_reba: {}", e))?;
