//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-SEBA`.
//! Source under test: `src/dict/abbr_seba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_seba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_seba.parquet
//!
//! `ICHIRAN/DICT:ABBR-SEBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:647`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_seba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_seba: {}", e))?;
