//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-ADV`.
//! Source under test: `src/dict/suffix_adv.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix::rules::suffix_adv;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-ADV";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_adv(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_adv: {}", e))?;
