//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-SA`.
//! Source under test: `src/dict/suffix_sa.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_sa_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_sa.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-SA` is a `def-simple-suffix`
//! (`dict-grammar.lisp:481`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_sa;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-SA";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_sa(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_sa: {}", e))?;
