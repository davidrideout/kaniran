//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-NAKEREBA`.
//! Source under test: `src/dict/abbr_nakereba.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix::abbr::abbr_nakereba;
...
const EXPECTED_FQN: &str = "ICHIRAN/DICT:ABBR-NAKEREBA";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_nakereba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_nakereba: {}", e))?;
