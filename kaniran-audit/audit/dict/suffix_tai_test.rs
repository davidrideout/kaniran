//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-TAI`.
//! Source under test: `src/dict/suffix_tai.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_tai_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_tai.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-TAI` is a `def-simple-suffix`
//! (`dict-grammar.lisp:370`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_tai;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-TAI";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_tai(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_tai: {}", e))?;
