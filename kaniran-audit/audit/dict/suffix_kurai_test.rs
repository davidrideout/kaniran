//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-KURAI`.
//! Source under test: `src/dict/suffix_kurai.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_kurai_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_kurai.parquet
//!
//! Replays a captured `(root, sv, kf)` triple through the
//! `suffix-kurai` `def-simple-suffix` (`dict-grammar.lisp:540`) and
...
use kaniran_core::dict::grammar::suffix::rules::suffix_kurai;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-KURAI";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_kurai(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_kurai: {}", e))?;
