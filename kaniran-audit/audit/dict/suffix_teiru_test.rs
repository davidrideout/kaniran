...
//! Source under test: `src/dict/suffix_teiru.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_teiru_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_teiru.parquet
//!
//! Args shape (`def-simple-suffix suffix-teiru :teiru (:connector ""
//! :score 3) (root)` at `dict-grammar.lisp:395` — macro signature
//! `(root sv kf)`):
//!   `[<root>, <sv>, <kf KANA-TEXT envelope>]`
//!
//! Result shape: `[<list> | null]`. `null` ↔ Lisp nil (teiru-check
...
//!
//! suffix-teiru is reached via `find-word-with-conj-type → find-word`
...
use kaniran_core::dict::grammar::suffix_rules::suffix_teiru;
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_teiru(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_teiru: {}", e))?;
