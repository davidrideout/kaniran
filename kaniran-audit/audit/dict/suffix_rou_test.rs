//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-ROU`.
//! Source under test: `src/dict/suffix_rou.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_rou_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_rou.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-ROU` is a `def-simple-suffix`
//! (`dict-grammar.lisp:461`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_rou;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-ROU";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_rou(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_rou: {}", e))?;
