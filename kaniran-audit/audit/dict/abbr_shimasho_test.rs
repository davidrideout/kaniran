//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-SHIMASHO`.
//! Source under test: `src/dict/abbr_shimasho.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_shimasho_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_shimasho.parquet
//!
//! `ICHIRAN/DICT:ABBR-SHIMASHO` reconstructs `root + "しましょう"` (destem
//! length 5) and runs `find-word-full` on it. Captured args are
//! `[<root>, <sv>, <suf | null>]`; the result is a list of word
//! envelopes or `null`.
...
use kaniran_core::dict::grammar::suffix::abbr::abbr_shimasho;
...
const EXPECTED_FQN: &str = "ICHIRAN/DICT:ABBR-SHIMASHO";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_shimasho(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_shimasho: {}", e))?;
