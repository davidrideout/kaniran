//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-CHAU`.
//! Source under test: `src/dict/suffix_chau.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_chau_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_chau.parquet
//!
//! Replays a captured `(root, sv, kf)` triple through the `suffix-chau`
//! `def-simple-suffix` (`dict-grammar.lisp:418`) and compares the
...
use kaniran_core::dict::grammar::suffix::rules::suffix_chau;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-CHAU";
...
fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_chau(ctx, root, sv, &kf)
        
        .map_err(|e| format!("suffix_chau: {}", e))?;
