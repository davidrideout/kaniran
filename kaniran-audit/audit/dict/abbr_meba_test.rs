//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-MEBA`.
//! Source under test: `src/dict/abbr_meba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_meba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_meba.parquet
//!
//! `ICHIRAN/DICT:ABBR-MEBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:644`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_meba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_meba: {}", e))?;
