//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-REN`.
//! Source under test: `src/dict/suffix_ren.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_ren_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_ren.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-REN` is a `def-simple-suffix`
//! (`dict-grammar.lisp:374`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_ren;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-REN";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_ren(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_ren: {}", e))?;
