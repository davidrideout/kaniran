//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-SOU+`.
//! Source under test: `src/dict/suffix_sou_plus_.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_sou_plus__test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_sou_plus_.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-SOU+` is a `def-simple-suffix`
//! (`dict-grammar.lisp:457`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_sou_plus_;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-SOU+";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_sou_plus_(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_sou_plus_: {}", e))?;
