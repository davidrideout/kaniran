//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-TE`.
//! Source under test: `src/dict/suffix_te.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_te_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_te.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-TE` is a `def-simple-suffix`
//! (`dict-grammar.lisp:389`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_te;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-TE";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_te(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_te: {}", e))?;
