...
//! Source under test: `src/dict/suffix_ren_.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix_rules::suffix_ren_;
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_ren_(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_ren_: {}", e))?;
