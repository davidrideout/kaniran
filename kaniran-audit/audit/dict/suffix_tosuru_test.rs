//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-TOSURU`.
//! Source under test: `src/dict/suffix_tosuru.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_tosuru_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_tosuru.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-TOSURU` is a `def-simple-suffix`
//! (`dict-grammar.lisp:537`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_tosuru;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-TOSURU";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_tosuru(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_tosuru: {}", e))?;
