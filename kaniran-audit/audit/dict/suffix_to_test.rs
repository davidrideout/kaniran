//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-TO`.
//! Source under test: `src/dict/suffix_to.rs`.
//!
//! Run with:
...
use kaniran_core::dict::grammar::suffix::rules::suffix_to;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-TO";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_to(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_to: {}", e))?;
