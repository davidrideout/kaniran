//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-IADJ`.
//! Source under test: `src/dict/suffix_iadj.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_iadj_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_iadj.parquet
//!
//! Replays a captured `(root, sv, kf)` triple through the `suffix-iadj`
//! `def-simple-suffix` (`dict-grammar.lisp:492`) and compares the
...
use kaniran_core::dict::grammar::suffix::rules::suffix_iadj;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-IADJ";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_iadj(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_iadj: {}", e))?;
