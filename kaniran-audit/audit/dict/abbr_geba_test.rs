//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-GEBA`.
//! Source under test: `src/dict/abbr_geba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_geba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_geba.parquet
//!
//! `ICHIRAN/DICT:ABBR-GEBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:635`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_geba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_geba: {}", e))?;
