//! Manual fixture-replay runner for `ICHIRAN/DICT:ABBR-KEBA`.
//! Source under test: `src/dict/abbr_keba.rs`.
//!
//! Run with:
//!   cargo run --release --bin abbr_keba_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/abbr_keba.parquet
//!
//! `ICHIRAN/DICT:ABBR-KEBA` is a `def-abbr-suffix`
//! (`dict-grammar.lisp:632`). The macro (`dict-grammar.lisp:547-579`)
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = abbr_keba(ctx, root, sv, None)
        .await
        .map_err(|e| format!("abbr_keba: {}", e))?;
