//! Manual fixture-replay runner for `ICHIRAN/DICT:SUFFIX-SUGIRU`.
//! Source under test: `src/dict/suffix_sugiru.rs`.
//!
//! Run with:
//!   cargo run --release --bin suffix_sugiru_test -- \
//!       --path corpus/extracted_chunk_c_suffix_abbr_2026_05_16/dict/suffix_sugiru.parquet
//!
//! `ICHIRAN/DICT:SUFFIX-SUGIRU` is a `def-simple-suffix`
//! (`dict-grammar.lisp:467`). The macro signature is `(root sv kf)`,
...
use kaniran_core::dict::grammar::suffix_rules::suffix_sugiru;

use common::{parse_captured_simple_text, parse_captured_word, CapturedRow};

const EXPECTED_FQN: &str = "ICHIRAN/DICT:SUFFIX-SUGIRU";
...
async fn audit_one(ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let actual = suffix_sugiru(ctx, root, sv, &kf)
        .await
        .map_err(|e| format!("suffix_sugiru: {}", e))?;
