//! Manual fixture-replay runner for `ICHIRAN/DICT:GET-SYNERGIES`.
//! Source under test: `src/dict/get_synergies.rs`.
//!
//! Run with:
...
const EXPECTED_FQN: &str = "ICHIRAN/DICT:GET-SYNERGIES";
...
async fn audit_one(_ctx: &KaniranContext, row: &CapturedRow) -> Result<(), String> {
    ...
    let lite_result = get_synergies(&lite_left, &lite_right);
    // Materialize lite path elements back to full PathElement for the
    ...
    compare_paths(&actual, expected_arr)
}

// ----- comparators ---------------------------------------------------------

fn compare_paths(
    actual: &[Vec<PathElement>],
    expected: &[Value],
) -> Result<(), String> {
    ...
    for (idx, (a_path, e_triple)) in actual.iter().zip(expected.iter()).enumerate() {
        let triple_arr = e_triple.as_array().ok_or_else(|| {
            format!("path[{}]: expected array, got {}", idx, e_triple)
        })?;
        if triple_arr.len() != 3 {
            return Err(format!(
                "path[{}]: expected 3 elements (right-sl, synergy, left-sl), got {}",
                idx,
                triple_arr.len()
            ));
        }
        if a_path.len() != 3 {
            return Err(format!(
                "path[{}]: rust path has {} elements, expected 3",
                idx,
                a_path.len()
            ));
        }
        let a_right = match &a_path[0] {
            PathElement::SegmentList(sl) => sl,
            PathElement::Synergy(_) => {
                return Err(format!("path[{}][0]: expected SegmentList, got Synergy", idx))
            }
        };
        let a_syn = match &a_path[1] {
            PathElement::Synergy(s) => s,
            PathElement::SegmentList(_) => {
                return Err(format!("path[{}][1]: expected Synergy, got SegmentList", idx))
            }
        };
        let a_left = match &a_path[2] {
            PathElement::SegmentList(sl) => sl,
            PathElement::Synergy(_) => {
                return Err(format!("path[{}][2]: expected SegmentList, got Synergy", idx))
            }
        };
        compare_segment_list_all(a_right, &triple_arr[0])
            .map_err(|err| format!("path[{}].right_sl: {}", idx, err))?;
        compare_synergy(a_syn, &triple_arr[1])
            .map_err(|err| format!("path[{}].synergy: {}", idx, err))?;
        compare_segment_list_all(a_left, &triple_arr[2])
            .map_err(|err| format!("path[{}].left_sl: {}", idx, err))?;
    }
    Ok(())
}
...
fn compare_info(actual: &KaniSegmentInfo, expected: &KaniSegmentInfo) -> Result<(), String> {
    ...
    // get-synergies is a pass-through dispatcher — neither it nor the
    // individual synergy fns mutate segment info. Comparing the full
    // info shape catches parse-side regressions in the runner itself.
