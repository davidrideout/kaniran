//! Port of `ichiran/dict:split-nakunaru2` (`dict-split.lisp:227`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1518540`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:227`).
//!
...
pub async fn split_nakunaru2(
    ...
    if !((r.word_type() == WordType::Kana)) {
        return Ok((parts, score));
    }

    {
        let pseq_lookup = find_word_conj_of(ctx, "亡く", &[1518450i32]).await?;
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1518540i32) {
            None
