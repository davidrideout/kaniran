//! Port of `ichiran/dict:split-hajikidasu` (`dict-split.lisp:340`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1419350`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:340`).
//!
...
pub async fn split_hajikidasu(
    ...
    {
        let pseq: &[i32] = &[1901710i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'き').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1419350i32) {
            None
