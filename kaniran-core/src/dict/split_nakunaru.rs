//! Port of `ichiran/dict:split-nakunaru` (`dict-split.lisp:223`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1529550`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:223`).
//!
...
pub async fn split_nakunaru(
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "無く", &[1529520i32]).await?;
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1529550i32) {
            None
