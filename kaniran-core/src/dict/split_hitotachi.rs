//! Port of `ichiran/dict:split-hitotachi` (`dict-split.lisp:344`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1368740`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:344`).
//!
...
pub async fn split_hitotachi(
    ...
    {
        let pseq: &[i32] = &[1580640i32];
        let part_length: Option<usize> = Some(if txt.chars().any(|c| c == '人') { 1usize } else { 2usize });
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1368740i32) {
            None
    ...
    {
        let pseq: &[i32] = &[1416220i32];
        let part_length: Option<usize> = Some(if txt.chars().any(|c| c == '達') { 1usize } else { 2usize });
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1368740i32) {
            None
        } else if let Some(pt) = part_txt {
