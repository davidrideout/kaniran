//! Port of `ichiran/dict:split-nitotte` (`dict-split.lisp:328`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1009600`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:328`).
//!
...
pub async fn split_nitotte(
...

    {
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1009600i32) {
            None
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "取って", &[1326980i32]).await?;
