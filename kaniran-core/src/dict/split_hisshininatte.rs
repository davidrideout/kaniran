//! Port of `ichiran/dict:split-hisshininatte` (`dict-split.lisp:323`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1903910`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:323`).
//!
...
pub async fn split_hisshininatte(
    ctx: &KaniranContext,
    ...
    {
        let pseq: &[i32] = &[1601890i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'に');
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1903910i32) {
            None
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "なって", &[1375610i32]).await?;
