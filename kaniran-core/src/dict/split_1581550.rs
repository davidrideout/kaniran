//! Port of `ichiran/dict:split-1581550` (`dict-split.lisp:596`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1581550`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:596`).
//!
...
pub async fn split_1581550(
    ...
    if !(txt.starts_with("雪")) {
        return Ok((parts, score));
    }

    {
        let pseq: &[i32] = &[1386500i32];
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1581550i32) {
            None
    ...
    if !((len_ as i32 > 2)) {
        score = -2;
        parts.push(Some(SplitPart::PScore));
        return Ok((parts, score));
    }
