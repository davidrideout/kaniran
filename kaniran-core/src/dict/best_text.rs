...
use crate::dict::conj_data::{get_conj_data, print_conj_info, ConjData, FromOrConjIds};
use crate::dict::counters::dispatchers::text;
use crate::dict::dao::{Entry, KanaText, KanjiText};
...
use crate::dict::segment::{TopArray, TopArrayItem};
use crate::dict::senses::{get_senses_str, reading_str_star_};
use crate::dict::text_classes::{ProxyText, WordConjugations};
use crate::dict::word_info::{
    simplify_reading_list, WordInfo, WordInfoKana, WordInfoSeq, WordInfoType, WordType,
};
...
pub fn true_text<'a>(obj: &'a KaniWordDispatchEnum) -> Cow<'a, str> {
    match obj {
        KaniWordDispatchEnum::Proxy(p) => Cow::Borrowed(unwrap_proxy_chain_text(p)),
        other => text(other),
    }
}

fn unwrap_proxy_chain_text(start: &ProxyText) -> &str {
