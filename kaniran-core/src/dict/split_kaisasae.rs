//! Port of `ichiran/dict:split-kaisasae` (`dict-split.lisp:360`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1752860`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:360`).
//!
...
pub async fn split_kaisasae(
...

    {
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1752860i32) {
            None
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "支え", &[1310090i32]).await?;
