//! Port of `ichiran/dict:split-1502500` (`dict-split.lisp:442`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1502500`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:442`).
//!
...
pub async fn split_1502500(
...

    {
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1502500i32) {
            None
    ...
    {
        let pseq: &[i32] = &[1277450i32];
        let part_length: Option<usize> = Some(2usize);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1502500i32) {
            None
        } else if let Some(pt) = part_txt {
            let pt_modified: String = { let mut s = pt.clone(); crate::characters::unrendaku::unrendaku(&mut s); s };
            find_word_seq(ctx, &pt_modified, pseq).await?.first_word().map(SplitPart::Word)
        } else {
            None
        };
