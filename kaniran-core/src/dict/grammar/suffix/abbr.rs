...
pub fn abbr_nee(
    ...
    ?;
    def_abbr_suffix_body(&ctx_rebound, primary_words, root, suf_var, 2, None)
}
...
pub fn abbr_nx(
    ...
    let (primary_words, patch): (Vec<KaniWordDispatchEnum>, Option<(&str, &str)>) = if root == "せ"
    {
        ...
        let rows = find_word_conj_of(&ctx_rebound, "しない", &[1157170])?;
    ...
    def_abbr_suffix_body(&ctx_rebound, primary_words, root, suf_var, 2, patch)
}
...
pub fn abbr_nakereba(
    ...
    let primary_words = find_word_full(&ctx_rebound, &wordstr, false, None)?;
    def_abbr_suffix_body(&ctx_rebound, primary_words, root, suf_var, 4, None)
}
...
pub fn abbr_shimasho(
    ...
    let primary_words = find_word_full(&ctx_rebound, &wordstr, false, None)?;
    def_abbr_suffix_body(&ctx_rebound, primary_words, root, suf_var, 5, None)
}
