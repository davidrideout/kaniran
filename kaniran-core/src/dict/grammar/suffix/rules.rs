...
pub fn def_simple_suffix_body(
...
) -> Result<Vec<CompoundText>, crate::conn::KaniDbError> {
    ...
    for entry in primary_words {
        ...
        let pw_kana = get_kana(ctx, &pw)?.unwrap_or_default();
...
pub fn suffix_tai(
    ...
    def_simple_suffix_body(ctx, primary_words, root, suffix, kf, &opts)
}
...
pub fn suffix_chau(
    ...
    def_simple_suffix_body(&ctx_rebound, primary_words, root, suf, kf, &opts)
}
...
pub fn suffix_suru(
    ...
    let primary_words: Vec<PrimaryWord> = match find_word_with_pos(ctx, root, &["vs"])? {
...
pub fn suffix_sou_base_body(
    ctx: &KaniranContext,
    root: &str,
) -> Result<(Vec<PrimaryWord>, Option<(&'static str, &'static str)>), crate::conn::KaniDbError> {
    ...
    if root.ends_with("なさ") {
        ...
        let words = find_word_with_conj_type(ctx, root, &[13, 51, 50])?;
        let primary_words = words.into_iter().map(PrimaryWord::from).collect();
...
pub fn suffix_sou(
    ...
    let (primary_words, patch) = suffix_sou_base_body(ctx, root)?;
    ...
    def_simple_suffix_body(ctx, primary_words, root, suf, kf, &opts)
}
    ...
    let primary_words: Vec<PrimaryWord> = match new_root_opt {
        ...
        Some(new_root) => {
            if patch_set.is_some() && new_root.chars().count() > 2 {
                ...
                match find_word_with_pos(&ctx_rebound, &new_root, &["adj-i"])? {
                    WordWithPosRows::Kana(rows) => rows
                        .into_iter()
                        .map(|r| PrimaryWord::from(KaniWordDispatchEnum::Kana(r)))
                        .collect(),
...
pub fn suffix_sa(
    ...
    let adj_na = match find_word_with_pos(ctx, root, &["adj-na"])? {
        WordWithPosRows::Kana(rows) => rows
            .into_iter()
            .map(|r| PrimaryWord::from(KaniWordDispatchEnum::Kana(r)))
            .collect::<Vec<_>>(),
...
pub fn suffix_garu(
    ...
    let (primary_words, patch_set): (Vec<PrimaryWord>, Option<(&'static str, &'static str)>) =
        if matches!(root, "な" | "い" | "よ") {
            ...
            // dict-errata.lisp:1237 — (defconstant +conj-adjective-stem+ 51)
            let arm_a = find_word_with_conj_type(ctx, root, &[51])?;
            if !arm_a.is_empty() {
                ...
                let new_root = apply_patch(root, patch);
                let ctx_inner = ctx.with_suffix_map_temp(None);
                let words = find_word_with_suffix(&ctx_inner, &new_root, &["sou"])?;
                (
...
pub fn suffix_ra(
    ...
    let primary_words: Vec<PrimaryWord> = if root.ends_with('ら') {
        ...
        let finder: OrAsHiraganaFinder<'_> = Arc::new(|word: String| {
            let rows = find_word_with_pos(ctx, &word, &["pn"])?;
            Ok(match rows {
        ...
        let words: Vec<KaniWordDispatchEnum> = match from_pn {
            Some(words) => words,
            None => match find_word_seq(ctx, root, &[1580640])? {
