//! Port of `ichiran/dict:split-de-2136520` (`dict-split.lisp:128`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2136520`.
//! Generated upstream by `def-de-split` (`dict-split.lisp:128`).
//!
...
pub async fn split_de_2136520(
    ...
    {
        let pseq: &[i32] = &[2005870i32];
        let part_length: Option<usize> = Some(((len_ as i32 - 1)).max(0) as usize);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2136520i32) {
            None
