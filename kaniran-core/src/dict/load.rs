...
/// Lexicographic comparator parameterised on element-level
/// `predicate`. Walks two sequences in lockstep; the first pair where
/// `predicate(e1, e2)` is true returns `true`, the first pair where
/// `predicate(e2, e1)` is true returns `false`. Mismatched lengths
/// walk the shared prefix then return `false`, matching CL's
/// `(map nil …)` semantics.
