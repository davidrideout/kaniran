//! Port of `ichiran/dict:split-2002270` (`dict-split.lisp:579`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2002270`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:579`).
//!
...
pub async fn split_2002270(
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "零れ", &[1557650i32]).await?;
        let pseq_vec: Vec<i32> = pseq_lookup.first_seq().into_iter().collect();
        let pseq: &[i32] = &pseq_vec;
        let part_length: Option<usize> = txt.chars().position(|c| c == 'れ').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2002270i32) {
            None
