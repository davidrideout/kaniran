//! Port of `ichiran/dict:split-tonattara` (`dict-split.lisp:373`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2100770`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:373`).
//!
...
pub async fn split_tonattara(
...

    {
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&2100770i32) {
            None
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "なったら", &[1375610i32]).await?;
