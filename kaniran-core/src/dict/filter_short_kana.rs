...
//! - `(get-text seg)` reads cached [`Segment::text`] when present,
//!   else [`get_text`]`(&seg.word)`; the closure takes `&SegmentList`
//!   so it cannot populate the cache slot the upstream defmethod
//!   would `setf`.
...
pub fn filter_short_kana(len: usize, except: Vec<String>) -> impl Fn(&SegmentList) -> bool {
    move |segment_list: &SegmentList| -> bool {
        ...
        let kpcl_first = match &seg.info {
            Some(info) => info.kpcl.0,
            None => false,
        };
        if kpcl_first {
            return false;
        ...
        if !except.is_empty() {
            let seg_text: Cow<str> = match &seg.text {
                Some(t) => Cow::Borrowed(t.as_str()),
                None => get_text(&seg.word),
            };
            if except.iter().any(|e| e.as_str() == seg_text.as_ref()) {
                return false;
            }
        }
        true
...
mod tests {
    ...
    fn c11_only_first_seg_examined() {
        ...
        assert!(f(&sl(0, 1, vec![s_good, s_kpcl])));
    }
