//! Port of `ichiran/dict:split-nantokanaru` (`dict-split.lisp:301`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2104540`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:301`).
//!
...
pub async fn split_nantokanaru(
    ctx: &KaniranContext,
    ...
    {
        let pseq: &[i32] = &[1188420i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'か').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2104540i32) {
            None
