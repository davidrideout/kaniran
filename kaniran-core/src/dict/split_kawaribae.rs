//! Port of `ichiran/dict:split-kawaribae` (`dict-split.lisp:240`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1411570`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:240`).
//!
...
pub async fn split_kawaribae(
    ...
    {
        let pseq: &[i32] = &[1590770i32, 1510720i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'り').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1411570i32) {
            None
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "映え", &[1600620i32]).await?;
