//! Port of `ichiran/dict:split-2529050` (`dict-split.lisp:607`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2529050`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:607`).
//!
...
pub async fn split_2529050(
    ...
    {
        let pseq: &[i32] = &[1322990i32];
        let part_length: Option<usize> = Some(if txt.starts_with("もの") { 2usize } else { 1usize });
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2529050i32) {
            None
