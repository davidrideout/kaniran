//! Port of `ichiran/dict:split-hayaimonode` (`dict-split.lisp:244`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2815260`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:244`).
//!
...
pub async fn split_hayaimonode(
    ctx: &KaniranContext,
    ...
    {
        let pseq: &[i32] = &[1404975i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'い').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2815260i32) {
            None
    ...
    {
        let pseq: &[i32] = &[1502390i32];
        let part_length: Option<usize> = Some(if txt.contains('物') { 1usize } else { 2usize });
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2815260i32) {
            None
        } else if let Some(pt) = part_txt {
