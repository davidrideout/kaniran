...
mod tests {
    ...
    fn r_a_l_nil_r_ku_pass_through() {
        ...
        let r = sl(0, 1, vec![simple_seg(0, 1, "くる", 100)]);
    ...
    fn r_b_l_nil_r_not_ku() {
        ...
        let r = sl(0, 1, vec![simple_seg(0, 1, "あさ", 100)]);
        let result = segfilter_roku(None, &r);
        assert_eq!(result.len(), 1);
    }
    ...
    fn r_c_l_simple_r_ku_pass_through() {
        ...
        let l = sl(0, 1, vec![simple_seg(0, 1, "abc", 999)]);
        let r = sl(1, 2, vec![simple_seg(1, 2, "くる", 100)]);
    ...
    fn r_d_l_iro_r_ku_empty() {
        ...
        let l = sl(0, 2, vec![compound_ending_seg(0, 2, "いろ", 50)]);
        let r = sl(2, 3, vec![simple_seg(2, 3, "くる", 100)]);
        let result = segfilter_roku(Some(&l), &r);
