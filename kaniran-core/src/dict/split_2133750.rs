//! Port of `ichiran/dict:split-2133750` (`dict-split.lisp:631`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2133750`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:631`).
//!
...
pub async fn split_2133750(
    ...
    {
        let pseq: &[i32] = &[1224890i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'く').map(|p| p + 1);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2133750i32) {
            None
