//! Port of `ichiran/dict:split-nara` (`dict-split.lisp:421`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1009470`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:421`).
//!
...
pub async fn split_nara(
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "なら", &[2089020i32]).await?;
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1009470i32) {
            None
