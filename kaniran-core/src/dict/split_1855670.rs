//! Port of `ichiran/dict:split-1855670` (`dict-split.lisp:683`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1855670`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:683`).
//!
...
pub async fn split_1855670(
...

    {
        ...
        let part_length: Option<usize> = txt.chars().position(|c| c == 'の');
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&1855670i32) {
            None
