//! Port of `ichiran/dict:split-toiukotoda` (`dict-split.lisp:368`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2612990`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:368`).
//!
...
pub async fn split_toiukotoda(
...

    {
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&2612990i32) {
            None
    ...
    {
        let pseq: &[i32] = &[1313580i32];
        let part_length: Option<usize> = Some(((len_ as i32 - 4)).max(0) as usize);
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2612990i32) {
            None
        } else if let Some(pt) = part_txt {
