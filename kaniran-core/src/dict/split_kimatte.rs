//! Port of `ichiran/dict:split-kimatte` (`dict-split.lisp:294`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1951150`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:294`).
//!
...
pub async fn split_kimatte(
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "決まって", &[1591420i32]).await?;
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1951150i32) {
            None
