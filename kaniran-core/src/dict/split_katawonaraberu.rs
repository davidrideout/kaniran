//! Port of `ichiran/dict:split-katawonaraberu` (`dict-split.lisp:285`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2102910`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:285`).
//!
...
pub async fn split_katawonaraberu(
    ctx: &KaniranContext,
    ...
    {
        let pseq: &[i32] = &[1258950i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'を');
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2102910i32) {
            None
