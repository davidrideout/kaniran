//! Port of `ichiran/dict:split-hairikomeru` (`dict-split.lisp:313`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `1465460`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:313`).
//!
...
pub async fn split_hairikomeru(
    ctx: &KaniranContext,
    ...
    {
        let pseq_lookup = find_word_conj_of(ctx, "入り", &[1465590i32]).await?;
        ...
        let pushed: Option<SplitPart> = if pseq.contains(&1465460i32) {
            None
