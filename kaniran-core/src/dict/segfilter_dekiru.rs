...
mod tests {
    ...
    fn de_a_l_nil_r_all_match() {
        ...
        let r = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![2830009])))]));
    ...
    fn de_c_l_de_r_no_match() {
        ...
        let l = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![1896380])))]));
        let r = Arc::new(sl(1, 2, vec![seg(1, 2, Some(info_with_seq_set(vec![999])))]));
    ...
    fn de_d_l_sat_l_r_sat_r() {
        ...
        let r = Arc::new(sl(1, 2, vec![seg(1, 2, Some(info_with_seq_set(vec![2830009])))]));
        let result = segfilter_dekiru(Some(&l), &r);
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].0.as_ref().unwrap().segments.len(), 1);
    }
