...
//! enumerates the `(kanji-text.id, conjugation.id)` pairs whose
//! `kanji-text` row is the parent reading of the `conj-source-reading`
//! that produced `(seq, text)`. Joins three tables: a `conjugation`
//! whose `seq` is the input `seq`, its `conj-source-reading` whose
//! `text` is the input `text`, and the `kanji-text` row matching the
