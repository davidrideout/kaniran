...
mod tests {
    ...
    fn ti_a_l_nil_r_iru_pass_through() {
        let r = sl(0, 1, vec![seg(0, 1, info_with_seq_set(vec![1577980]))]);
    ...
    fn ti_c_l_not_tsu_r_iru() {
        ...
        let r = sl(1, 2, vec![seg(1, 2, info_with_seq_set(vec![1577980]))]);
    ...
    fn ti_d_l_tsu_r_iru_empty() {
        let l = sl(0, 1, vec![seg(0, 1, info_with_seq_set(vec![2221640]))]);
        let r = sl(1, 2, vec![seg(1, 2, info_with_seq_set(vec![1577980]))]);
        let result = segfilter_tsu_iru(Some(&l), &r);
