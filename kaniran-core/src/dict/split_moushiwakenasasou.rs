//! Port of `ichiran/dict:split-moushiwakenasasou` (`dict-split.lisp:290`).
//!
//! Registered in [`crate::dict::_star_split_map_star_`] for seq `2057340`.
//! Generated upstream by `def-simple-split` (`dict-split.lisp:290`).
//!
...
pub async fn split_moushiwakenasasou(
    ctx: &KaniranContext,
    ...
    {
        let pseq: &[i32] = &[1363050i32];
        let part_length: Option<usize> = txt.chars().position(|c| c == 'な');
        let part_txt = crate::characters::safe_subseq::safe_subseq(&txt, offset, part_length.map(|pl| offset + pl));
        let pushed: Option<SplitPart> = if pseq.contains(&2057340i32) {
            None
