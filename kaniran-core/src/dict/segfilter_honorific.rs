...
mod tests {
    ...
    fn h_a_l_nil_r_all_honor_empty() {
        ...
        let r = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![1247260])))]));
    ...
    fn h_c_l_nil_r_no_match() {
        ...
        let r = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![999])))]));
        let result = segfilter_honorific(None, &r);
        assert_eq!(result.len(), 1);
        assert!(result[0].0.is_none());
        assert_eq!(result[0].1.segments.len(), 1);
    ...
    fn h_d_l_not_noun_r_honor() {
        ...
        let r = Arc::new(sl(1, 2, vec![seg(1, 2, Some(info_with_seq_set(vec![1247260])))]));
    ...
    fn h_e_l_is_noun_r_honor_empty() {
        ...
        let l = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![2028920])))]));
        let r = Arc::new(sl(1, 2, vec![seg(1, 2, Some(info_with_seq_set(vec![1247260])))]));
        let result = segfilter_honorific(Some(&l), &r);
        assert!(result.is_empty());
    }
    ...
    fn h_i_gap_r_all_honor_empty() {
        ...
        let l = Arc::new(sl(0, 1, vec![seg(0, 1, Some(info_with_seq_set(vec![999])))]));
        let r = Arc::new(sl(2, 3, vec![seg(2, 3, Some(info_with_seq_set(vec![1247260])))]));
        let result = segfilter_honorific(Some(&l), &r);
        assert!(result.is_empty());
    }
}
